...
public class MainFrame extends JFrame {
	...
	private void sendData(java.awt.event.ActionEvent evt) {
		...
		//待发送数据转换为bytes,串口数据只用3个字节，bytes[0]（字节数） bytes【1】地址 bytes[2] 服务状态1呼叫 0 取消
        byte[] bytes=new byte[3];

            String str = data.substring(0,  8);
            bytes[0]=(byte) Integer.parseInt(str,2);
            bytes[1]=(byte) Integer.parseInt(data.substring(8,16),2);
            bytes[2]=(byte) Integer.parseInt(data.substring(16,24),2);
		...
		// 以字符串的形式发送数据
		if (mDataASCIIChoice.isSelected()) {
			SerialPortManager.sendToPort(mSerialport, bytes);
		}
